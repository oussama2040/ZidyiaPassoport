...
export const countPendingDocs = async (req, res) => {
    try {
        const query = 'SELECT COUNT(*) AS PendingDocs FROM filledforms WHERE status = "pending"';
        db.query(query, (error, results) => {
            if (error) {
                console.error('Error querying database:', error);
                return res.status(500).json({ error: 'Database error' });
...
export const countApprovedDocs = async (req, res) => {
    try {
        const query = 'SELECT COUNT(*) AS ApprovedDocs FROM filledforms WHERE status = "verified"';
        db.query(query, (error, results) => {
            if (error) {
                console.error('Error querying database:', error);
                return res.status(500).json({ error: 'Database error' });
...
export const countRejectedDocs = async (req, res) => {
    try {
        const query = 'SELECT COUNT(*) AS RejectedDocs FROM filledforms WHERE status = "rejected"';
        db.query(query, (error, results) => {
            if (error) {
                console.error('Error querying database:', error);
                return res.status(500).json({ error: 'Database error' });
...
export const CreateVerifier = async (req, res) => {
    try {
        ...
        // Check if email already exists
        const emailExistsQuery = 'SELECT COUNT(*) AS count FROM subscriber WHERE admin_email = ?';
        db.query(emailExistsQuery, [admin_email], async (error, results) => {
            if (error) {
                console.error('Error querying database:', error);
                return res.status(500).json({ error: 'Database error' });
            ...
            const endDate = new Date();
            endDate.setFullYear(endDate.getFullYear() + 1);
            const query = 'INSERT INTO subscriber (admin_email, password, name, location, expiry_date) VALUES (?, ?, ?, ?, ?)';
            db.query(query, [admin_email, hashedPassword, name, location, endDate], (error, results) => {
                if (error) {
                    console.error('Error querying database:', error);
                    return res.status(500).json({ error: 'Database error' });
                }
                res.status(200).json({
                    message: "Verifier created successfully", results
                });
                sendCreateSubscriberNotification(admin_email, autoGeneratedPassword);
            });
        });
    } catch (error) {
        console.error('Error querying database:', error);
